...
class Promotion
{
    ...
    {
        return $this->promotion_rate . ' '.$this->promotion_start->format('d/m/Y').' '.$this->promotion_end->format('d/m/Y');
    }
    ...
    public function applyPromotionPrice(Product $product): ?string
    {
        if (!$this->productCanNotBePromotedAtTheSameTime($product)) {
            $promotionPrice =  $product->getDiscountedPrice($this);
