...
class Promotion
{
    ...
    {
        $promotionPrice = $product->getProductPrice() * (1 - $this->getPromotionRate() / 100);
        return number_format($promotionPrice, 2, ',', ' ');
    }
    ...
    public function productCanNotBePromotedAtTheSameTime(Product $product): bool
    {
        foreach ($product->getPromotions() as $promotion) {
            if ($this->dateBetweenPromotion() && $promotion->dateBetweenPromotion()) {
                return true;
            }
        }
        return false;
